...
import { usePasskeyContext, LoginPhase, RegistrationPhase, RegistrationStatus } from '@tatchi-xyz/sdk/react'
...
export default function HomeClient() {
  ...
  const onRegister = React.useCallback(async () => {
    ...
    const result = await registerPasskey(targetAccountId, {
      onEvent: (event: any) => {
        if (event.phase === RegistrationPhase.STEP_7_REGISTRATION_COMPLETE && event.status === RegistrationStatus.SUCCESS) {
