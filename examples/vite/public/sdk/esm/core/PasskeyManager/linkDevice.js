...
import { getDeviceLinkingAccountContractCall } from "../rpcCalls.js";
import { require_browser } from "../../node_modules/.pnpm/qrcode@1.5.4/node_modules/qrcode/lib/browser.js";
...
async function generateQRCodeDataURL(data) {
	return import_browser.toDataURL(data, {
		width: 256,
		margin: 2,
		color: {
			dark: "#000000",
...
var LinkDeviceFlow = class {
	...
	pollingInterval;
	pollGeneration = 0;
	KEY_POLLING_INTERVAL = DEVICE_LINKING_CONFIG.TIMEOUTS.POLLING_INTERVAL_MS;
	registrationRetryTimeout;
	registrationRetryCount = 0;
	MAX_REGISTRATION_RETRIES = DEVICE_LINKING_CONFIG.RETRY.MAX_REGISTRATION_ATTEMPTS;
	RETRY_DELAY_MS = DEVICE_LINKING_CONFIG.TIMEOUTS.REGISTRATION_RETRY_DELAY_MS;
	tempKeyCleanupTimer;
	TEMP_KEY_CLEANUP_DELAY_MS = DEVICE_LINKING_CONFIG.TIMEOUTS.TEMP_KEY_CLEANUP_MS;
	...
	*/
	async generateQR(accountId) {
		try {
			...
			throw new DeviceLinkingError(`Failed to generate device linking QR: ${error.message}`, DeviceLinkingErrorCode.REGISTRATION_FAILED, "generation");
		}
	}
	/**
	...
	*/
	...
	shouldContinuePolling() {
		...
		if (this.session.phase === DeviceLinkingPhase.STEP_5_ADDKEY_DETECTED) return false;
		if (this.session.phase === DeviceLinkingPhase.STEP_7_LINKING_COMPLETE) return false;
	...
	*/
	async checkForDeviceKeyAdded() {
		...
		try {
			const linkingResult = await getDeviceLinkingAccountContractCall(this.context.nearClient, this.context.configs.contractId, this.session.nearPublicKey);
			if (this.cancelled || !this.pollingInterval) return false;
			this.safeOnEvent({
				step: 4,
				phase: DeviceLinkingPhase.STEP_4_POLLING,
	...
	*/
	async executeKeySwapTransaction(newPublicKey, nextNonce, txBlockHash) {
		...
		try {
			...
			const txResult = await this.context.nearClient.sendTransaction(keySwapResult.signedTransaction, DEFAULT_WAIT_STATUS.linkDeviceSwapKey);
