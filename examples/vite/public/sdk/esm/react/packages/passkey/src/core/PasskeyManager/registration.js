...
import { RegistrationPhase, RegistrationStatus } from "../types/passkeyManager.js";
import { createAccountAndRegisterWithRelayServer } from "./faucets/createAccountRelayServer.js";
import { createAccountAndRegisterWithTestnetFaucet } from "./faucets/createAccountTestnetFaucet.js";
...
async function registerPasskey(context, nearAccountId, options, authenticatorOptions) {
	...
	try {
		...
		if (useRelayer) {
			...
			accountAndRegistrationResult = await createAccountAndRegisterWithRelayServer(context, nearAccountId, nearKeyResult.publicKey, credential, vrfChallenge, deterministicVrfKeyResult.vrfPublicKey, authenticatorOptions, onEvent);
		} else {
			console.debug("Using testnet faucet registration flow");
			accountAndRegistrationResult = await createAccountAndRegisterWithTestnetFaucet(context, nearAccountId, nearKeyResult.publicKey, credential, vrfChallenge, deterministicVrfKeyResult.vrfPublicKey, authenticatorOptions, onEvent);
		}
...
//#endregion
export { generateBootstrapVrfChallenge, registerPasskey };
