import { __toDynamicImportESM } from "../../../../_virtual/rolldown_runtime.js";
import { DeviceLinkingError, DeviceLinkingErrorCode } from "../core/types/linkDevice.js";
import { validateDeviceLinkingQRData } from "../core/PasskeyManager/scanDevice.js";
...
var ScanQRCodeFlow = class {
	...
	*/
	...
	async scanQRFromImageData(imageData) {
		const { default: jsQR } = await import("../../../../node_modules/.pnpm/jsqr@1.4.0/node_modules/jsqr/dist/jsQR.js").then(__toDynamicImportESM(1));
...
async function scanQRCodeFromFile(file) {
	...
	const dataUrl = await new Promise((resolve, reject) => {
		...
		reader.onload = (e) => {
			...
			else reject(createQRError("Failed to read file"));
		};
	...
	if (!qrData) throw createQRError("No QR code found in image");
...
*/
function detectFrontCamera(camera) {
	const label = camera.label.toLowerCase();
	return label.includes("front") || label.includes("user") || label.includes("selfie") || label.includes("facetime") || label.includes("facing front");
}
async function scanQRFromImageData(imageData) {
	const { default: jsQR } = await import("../../../../node_modules/.pnpm/jsqr@1.4.0/node_modules/jsqr/dist/jsQR.js").then(__toDynamicImportESM(1));
	const code = jsQR(imageData.data, imageData.width, imageData.height, { inversionAttempts: "dontInvert" });
	return code ? code.data : null;
}
function parseAndValidateQRData(qrData) {
	let parsedData;
	try {
		parsedData = JSON.parse(qrData);
	} catch {
...
function createQRError(message) {
	return new DeviceLinkingError(message, DeviceLinkingErrorCode.INVALID_QR_DATA, "authorization");
}

//#endregion
export { ScanQRCodeFlow, ScanQRCodeFlowState, detectFrontCamera, enumerateVideoDevices, scanQRCodeFromFile };
