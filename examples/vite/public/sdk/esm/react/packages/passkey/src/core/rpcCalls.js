...
import { ActionPhase, DeviceLinkingPhase, DeviceLinkingStatus } from "./types/passkeyManager.js";
...
async function executeDeviceLinkingContractCalls({ context, device1AccountId, device2PublicKey, nextNonce, nextNextNonce, nextNextNextNonce, txBlockHash, vrfChallenge, onEvent }) {
	const signedTransactions = await context.webAuthnManager.signTransactionsWithActions({
		...
		onEvent: (progress) => {
			if (progress.phase == ActionPhase.STEP_7_TRANSACTION_SIGNING_COMPLETE) onEvent?.({
				step: 3,
	...
	try {
		...
		addKeyTxResult = await context.nearClient.sendTransaction(signedTransactions[0].signedTransaction, DEFAULT_WAIT_STATUS.linkDeviceAddKey);
		...
		storeDeviceLinkingTxResult = await context.nearClient.sendTransaction(contractTx, DEFAULT_WAIT_STATUS.linkDeviceAccountMapping);
	} catch (txError) {
...
//#endregion
export { executeDeviceLinkingContractCalls, getCredentialIdsContractCall, getDeviceLinkingAccountContractCall, syncAuthenticatorsContractCall };
