...
import { Shamir3PassUtils } from "./shamirWorker.js";
...
var AuthService = class {
	...
	async _ensureSignerAndRelayerAccount() {
		...
		if (!this.shamir3pass) this.shamir3pass = new Shamir3PassUtils({
		...
		return this.queueTransaction(async () => {
			try {
				...
				const result = await this.relayerAccount.signAndSendTransaction({
					...
					actions: [actionCreators.functionCall("create_account_and_register_user", contractArgs, BigInt(this.config.createAccountAndRegisterGas), BigInt(this.config.accountInitialBalance))]
				});
