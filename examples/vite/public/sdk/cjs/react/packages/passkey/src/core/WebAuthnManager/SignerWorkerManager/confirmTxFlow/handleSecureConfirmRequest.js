...
const require_tags = require('../../LitComponents/IframeButtonWithTooltipConfirmer/tags.js');
const require_modal = require('../../LitComponents/modal.js');
...
async function renderUserConfirmUI({ ctx, data, confirmationConfig, transactionSummary, vrfChallenge }) {
	switch (confirmationConfig.uiMode) {
		...
		case "embedded": try {
			const hostEl = document.querySelector(require_tags.IFRAME_BUTTON_ID);
		...
		case "modal": if (confirmationConfig.behavior === "autoProceed") {
			const handle = await require_modal.mountIframeModalHostWithHandle({
				ctx,
			...
			const { confirmed, handle } = await require_modal.awaitIframeModalDecisionWithHandle({
				ctx,
				summary: transactionSummary,
				txSigningRequests: data.tx_signing_requests,
				vrfChallenge,
...
async function collectTouchIdCredentials({ ctx, data, decision }) {
	...
	const serializedCredential = data.isRegistration ? require_credentialsHelpers.serializeRegistrationCredentialWithPRF({
		credential,
		firstPrfOutput: true,
		secondPrfOutput: true
	}) : require_credentialsHelpers.serializeAuthenticationCredentialWithPRF({ credential });
