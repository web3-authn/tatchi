const require_tags = require('./IframeButtonWithTooltipConfirmer/tags.js');

//#region src/core/WebAuthnManager/LitComponents/modal.ts
async function ensureIframeModalDefined() {
	if (customElements.get(require_tags.IFRAME_MODAL_ID)) return;
	await new Promise((resolve, reject) => {
		const existing = document.querySelector(`script[data-w3a="${require_tags.IFRAME_MODAL_ID}"]`);
		if (existing) {
		...
		script.type = "module";
		script.async = true;
		script.dataset.w3a = require_tags.IFRAME_MODAL_ID;
		script.src = `/sdk/embedded/${require_tags.IFRAME_MODAL_ID}.js`;
...
async function mountIframeModalHostWithHandle({ ctx, summary, txSigningRequests, vrfChallenge, loading, theme }) {
	...
	const el = document.createElement(require_tags.IFRAME_MODAL_ID);
...
//#endregion
exports.awaitIframeModalDecisionWithHandle = awaitIframeModalDecisionWithHandle;
exports.mountIframeModalHostWithHandle = mountIframeModalHostWithHandle;
