...
const require_tx_tree_themes = require('../TxTree/tx-tree-themes.js');
...
const require_actions = require('../../../PasskeyManager/actions.js');
const require_button_with_tooltip_themes = require('./button-with-tooltip-themes.js');
const require_iframe_geometry = require('./iframe-geometry.js');
...
*/
var IframeButtonHost = class extends require_LitElementWithProps.LitElementWithProps {
	static properties = {
		nearAccountId: {
			type: String,
  ...
  `;
	...
	iframeRef = require_ref.e();
	hostRef = require_ref.e();
	...
	render() {
      ...
      <div class="iframe-button-host" ${require_ref.n(this.hostRef)}
        style="width: ${require_iframe_geometry.toPx(buttonSize.width)}; height: ${require_iframe_geometry.toPx(buttonSize.height)};"
      >
        <div class="host-button-visual"><slot>${this.buttonTextElement}</slot></div>
        <iframe
          ${require_ref.n(this.iframeRef)}
	...
	calculateIframeSize() {
		const buttonWidth = require_iframe_geometry.utilParsePx(this.buttonStyle?.width || "200px");
		const buttonHeight = require_iframe_geometry.utilParsePx(this.buttonStyle?.height || "48px");
		const tooltipWidth = require_iframe_geometry.utilParsePx(this.tooltipPosition.width);
		const tooltipHeight = this.tooltipPosition.height === "auto" ? 200 : require_iframe_geometry.utilParsePx(this.tooltipPosition.height);
		const offset = require_iframe_geometry.utilParsePx(this.tooltipPosition.offset);
		return require_iframe_geometry.computeIframeSizePure({
	...
	buildInitData() {
		...
		return {
			size: {
				width: require_iframe_geometry.toPx(buttonSize.width),
				height: require_iframe_geometry.toPx(buttonSize.height)
			},
			tooltip: {
				width: require_iframe_geometry.toPx(this.tooltipPosition.width),
				height: this.tooltipPosition.height,
				position: this.tooltipPosition.position,
				offset: require_iframe_geometry.toPx(this.tooltipPosition.offset)
			},
			buttonPosition: {
			...
			tagName: require_tags.BUTTON_WITH_TOOLTIP_ID,
			targetOrigin: window.location.origin
		};
	}
	generateIframeHtml() {
		const embeddedTxButtonTag = require_tags.BUTTON_WITH_TOOLTIP_ID;
		const iframeBootstrapTag = require_tags.IFRAME_BOOTSTRAP_MODULE;
		const base = require_tags.EMBEDDED_SDK_BASE_PATH;
		return `<!DOCTYPE html>
      <html>
        <head>
          <meta charset="utf-8" />
	...
	postStyleUpdateToIframe() {
		...
		const embeddedButtonTheme = require_button_with_tooltip_themes.EMBEDDED_TX_BUTTON_THEMES[this.txTreeTheme || "dark"];
	...
	getThemeStyles(theme) {
		return require_tx_tree_themes.TX_TREE_THEMES[theme] || require_tx_tree_themes.TX_TREE_THEMES.dark;
	}
	...
	*/
	applyButtonOnlyClipPath() {
		...
		const buttonClipPath = require_iframe_geometry.IframeClipPathGenerator.buildButtonClipPathPure(button, 4);
	...
	*/
	applyButtonTooltipClipPath() {
		...
		if (!this.clipPathSupported) return;
		try {
			const unionClipPath = require_iframe_geometry.IframeClipPathGenerator.generateUnion(this.currentGeometry, 4);
			if (unionClipPath) {
	...
	handleTooltipState(geometry) {
		...
		if (!geometry.visible) {
			...
			if (iframe) {
				...
				const size = require_iframe_geometry.computeExpandedIframeSizeFromGeometryPure({
					geometry,
					fallback,
					paddingPx: 0
				});
	...
	*/
	...
	async handleConfirm() {
		...
		try {
			const txResults = await require_actions.signAndSendTransactionsInternal({
				context: this.passkeyManagerContext,
				nearAccountId: require_accountIds.toAccountId(this.nearAccountId),
...
customElements.define(require_tags.IFRAME_BUTTON_ID, IframeButtonHost);

//#endregion
exports.IframeButtonHost = IframeButtonHost;
