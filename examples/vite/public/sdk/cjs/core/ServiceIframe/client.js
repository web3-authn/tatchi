const require_sanitization = require('./sanitization.js');
...
var ServiceIframeClient = class {
	...
	mountHiddenIframe() {
		...
		if (this.opts.walletOrigin) {
			...
			let serviceHostUrl = "";
			try {
				serviceHostUrl = new URL("../../../embedded/service-host.js", require("url").pathToFileURL(__filename).href).toString();
			} catch {
				const sanitizedBasePath = require_sanitization.sanitizeSdkBasePath(this.opts.sdkBasePath);
				serviceHostUrl = `${sanitizedBasePath}/embedded/service-host.js`;
			}
			const escapedUrl = require_sanitization.escapeHtmlAttribute(serviceHostUrl);
...
//#endregion
exports.ServiceIframeClient = ServiceIframeClient;
