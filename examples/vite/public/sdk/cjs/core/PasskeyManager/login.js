...
async function loginPasskey(context, nearAccountId, options) {
	...
	onEvent?.({
		step: 1,
		phase: require_passkeyManager.LoginPhase.STEP_1_PREPARATION,
		status: require_passkeyManager.LoginStatus.PROGRESS,
	...
	try {
		...
		if (!window.isSecureContext) {
			...
			onEvent?.({
				step: 0,
				phase: require_passkeyManager.LoginPhase.LOGIN_ERROR,
				status: require_passkeyManager.LoginStatus.ERROR,
...
async function handleLoginUnlockVRF(context, nearAccountId, onEvent, onError, hooks) {
	...
	try {
		...
		onEvent?.({
			step: 2,
			phase: require_passkeyManager.LoginPhase.STEP_2_WEBAUTHN_ASSERTION,
			status: require_passkeyManager.LoginStatus.PROGRESS,
		...
		onEvent?.({
			step: 3,
			phase: require_passkeyManager.LoginPhase.STEP_3_VRF_UNLOCK,
			status: require_passkeyManager.LoginStatus.SUCCESS,
		...
		onEvent?.({
			step: 4,
			phase: require_passkeyManager.LoginPhase.STEP_4_LOGIN_COMPLETE,
			status: require_passkeyManager.LoginStatus.SUCCESS,
		...
		const errorMessage = require_errors.getUserFriendlyErrorMessage(error, "login");
		onError?.(error);
		onEvent?.({
			step: 0,
			phase: require_passkeyManager.LoginPhase.LOGIN_ERROR,
...
*/
async function verifyAuthenticationResponse(relayServerUrl, vrfChallenge, webauthnAuthentication) {
	try {
...
exports.logoutAndClearVrfSession = logoutAndClearVrfSession;
