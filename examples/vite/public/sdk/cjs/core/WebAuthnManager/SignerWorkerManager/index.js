...
const require_deriveNearKeypairAndEncrypt = require('./handlers/deriveNearKeypairAndEncrypt.js');
const require_decryptPrivateKeyWithPrf = require('./handlers/decryptPrivateKeyWithPrf.js');
const require_signVerifyAndRegisterUser = require('./handlers/signVerifyAndRegisterUser.js');
...
var SignerWorkerManager = class {
	...
	createSecureWorker() {
		const workerUrl = new URL(require_config.SIGNER_WORKER_MANAGER_CONFIG.WORKER.URL, window.location.origin);
	...
	*/
	...
	async sendMessage({ message, onEvent, timeoutMs = require_config.SIGNER_WORKER_MANAGER_CONFIG.TIMEOUTS.DEFAULT }) {
		...
		return new Promise((resolve, reject) => {
			...
			worker.onmessage = async (event) => {
				try {
					...
					if (event.data.type === require_types.SecureConfirmMessageType.PROMPT_USER_CONFIRM_IN_JS_MAIN_THREAD) {
						await require_handleSecureConfirmRequest.handlePromptUserConfirmInJsMainThread(this.getContext(), event.data, worker);
						return;
					}
	...
	*/
	async deriveNearKeypairAndEncrypt(args) {
		return require_deriveNearKeypairAndEncrypt.deriveNearKeypairAndEncrypt({
			ctx: this.getContext(),
			...args
		});
	}
	...
	*/
	async signVerifyAndRegisterUser(args) {
		return require_signVerifyAndRegisterUser.signVerifyAndRegisterUser({
			ctx: this.getContext(),
			...args
		});
	}
