...
const require_build_paths = require('../../../packages/passkey/build-paths.js');
...
var VrfWorkerManager = class {
	...
	constructor(config = {}) {
		this.config = {
			vrfWorkerUrl: require_build_paths.BUILD_PATHS.RUNTIME.VRF_WORKER,
			workerTimeout: 1e4,
			debug: false,
			...config
		};
	...
	*/
	async generateVrfChallenge(inputData) {
		...
		return require_vrf_worker.validateVRFChallenge(response.data);
	}
	/**
	...
	async checkVrfStatus() {
		...
		try {
			...
			if (response.success && response.data) return {
				...
				nearAccountId: this.currentVrfAccountId ? require_accountIds.toAccountId(this.currentVrfAccountId) : null,
	...
	async generateVrfKeypairBootstrap(vrfInputData, saveInMemory) {
		...
		try {
			...
			return {
				...
				vrfChallenge: require_vrf_worker.validateVRFChallenge({
	...
	*/
	async deriveVrfKeypairFromPrf({ credential, nearAccountId, vrfInputData, saveInMemory = true }) {
		...
		try {
			...
			const vrfChallenge = response.data.vrfChallengeData ? require_vrf_worker.validateVRFChallenge({
