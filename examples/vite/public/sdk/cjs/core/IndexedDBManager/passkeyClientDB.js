...
var PasskeyClientDBManager = class {
	...
	*/
	validateNearAccountId(nearAccountId) {
		return require_validation.validateNearAccountId(nearAccountId);
	}
	/**
	...
	*/
	async registerUser(storeUserData) {
		...
		const now = Date.now();
		const userData = {
			nearAccountId: require_accountIds.toAccountId(storeUserData.nearAccountId),
	...
	*/
	async getConfirmationConfig(nearAccountId) {
		const user = await this.getUser(nearAccountId);
		return user?.preferences?.confirmationConfig || require_signer_worker.DEFAULT_CONFIRMATION_CONFIG;
	}
	/**
