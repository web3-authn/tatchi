...
The SDK refreshes stored blobs after success and proactively migrates to the current server key when possible. See also [Shamir3Pass login flow notes (SDK docs)](https://github.com/web3-authn/sdk/blob/main/sdk/docs/shamir3pass-login.md) and the rotation details in [Shamir3Pass Rotation](./shamir3pass-rotate-keys).
