...
- Ensure `examples/wallet-scoped-credentials/vite.config.ts` keeps the Tatchi Vite plugins:
  - `tatchiDev(...)` for local serve
  - `tatchiBuildHeaders({ walletOrigin: env.VITE_WALLET_ORIGIN })` so `_headers` is emitted for Pages and `wallet-service/index.html` is written automatically if missing
...
- The dev plugin serves `GET /wallet-service`. In production, the build plugin writes `dist/wallet-service/index.html` automatically if you don’t provide one under `public/`. No extra CI step is required.
