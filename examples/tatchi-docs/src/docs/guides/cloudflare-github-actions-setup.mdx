...
- Ensure your Vite app includes the build plugin:
  - File: examples/vite-secure/vite.config.ts
  - Import: `tatchiBuildHeaders` from `@tatchi/sdk/plugins/vite`
  - Add: `tatchiBuildHeaders({ walletOrigin: env.VITE_WALLET_ORIGIN })`

- Provide env vars in the Pages project:
...
Outputs:
- `dist/_headers` with COOP/COEP, Permissions‑Policy, and permissive CORS for `${VITE_SDK_BASE_PATH}/*` and `${VITE_SDK_BASE_PATH}/workers/*`.
- `dist${VITE_WALLET_SERVICE_PATH}/index.html` if missing. The plugin won’t overwrite existing files under `public/`.

With this, CI doesn’t need to inject HTML or headers — just build and deploy the example’s `dist`.
