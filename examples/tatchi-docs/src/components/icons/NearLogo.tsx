import React from 'react'

export type NearLogoProps = React.SVGProps<SVGSVGElement> & {
  size?: number | string
  title?: string
}

/**
 * NEAR mark (icon only). Uses currentColor for fill so it can be themed.
 * Extracted from src/public/near-logo-dark.svg / near-logo-light.svg.
 */
const NearLogo: React.FC<NearLogoProps> = ({ size = 64, title, className, ...rest }) => {
  const ariaHidden = title ? undefined : true
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 639 639"
      width={size}
      height={typeof size === 'number' ? undefined : undefined}
      fill="currentColor"
      aria-hidden={ariaHidden}
      role={title ? 'img' : undefined}
      focusable={false}
      className={className}
      {...rest}
    >
      {title ? <title>{title}</title> : null}
      <path d="m457.61,144c-13,0-25.07,6.74-31.88,17.82l-73.37,108.93c-2.39,3.59-1.42,8.43,2.17,10.82,2.91,1.94,6.76,1.7,9.41-.58l72.22-62.64c1.2-1.08,3.05-.97,4.13.23.49.55.75,1.26.75,1.99v196.12c0,1.62-1.31,2.92-2.93,2.92-.87,0-1.69-.38-2.24-1.05L217.56,157.24c-7.11-8.39-17.55-13.23-28.54-13.24h-7.63c-20.65,0-37.39,16.74-37.39,37.39v276.22c0,20.65,16.74,37.39,37.39,37.39,13,0,25.07-6.74,31.88-17.82l73.37-108.93c2.39-3.59,1.42-8.43-2.17-10.82-2.91-1.94-6.76-1.7-9.41.58l-72.22,62.64c-1.2,1.08-3.05.97-4.13-.23-.49-.55-.75-1.26-.74-1.99v-196.17c0-1.62,1.31-2.92,2.93-2.92.86,0,1.69.38,2.24,1.05l218.28,261.37c7.11,8.39,17.55,13.23,28.54,13.24h7.63c20.65.01,37.4-16.72,37.42-37.37V181.39c0-20.65-16.74-37.39-37.39-37.39Z"/>
    </svg>
  )
}

export default NearLogo

