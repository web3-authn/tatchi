...
import type { UseThemeReturn, DesignTokens } from '../ProfileSettingsButton/types';
import { useThemeContext } from './ThemeContext';

export function useTheme(): UseThemeReturn {
  const ctx = useThemeContext();
  return useMemo(
    () => ({ theme: ctx.theme, tokens: ctx.tokens, isDark: ctx.isDark, toggleTheme: ctx.toggleTheme }),
    [ctx.theme, ctx.tokens, ctx.isDark, ctx.toggleTheme]
  );
}
...
export function useComponentTheme<T extends Record<string, DesignTokens>>(
  ...
  return { theme, tokens, isDark, toggleTheme };
}
