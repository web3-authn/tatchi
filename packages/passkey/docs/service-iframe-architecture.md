...
- Service Iframe (wallet origin):
  ...
  - Implements RPC handlers: on `REQUEST_SIGN` or `REQUEST_REGISTER` performs WebAuthn and calls local workers; returns only signature/signed txs.
...
1. Parent app constructs `PasskeyManager` and (optionally) sets `walletOrigin` in configs. React’s provider auto‑inits; bare JS calls `initServiceIframe()`.
2. Parent mounts hidden service iframe:
   - Preferred (no external hosting): uses `srcdoc` with an imported module asset URL resolved by the consumer bundler (e.g., `service-host.ts?url`). This keeps the service same‑origin and sandboxed, without copying HTML.
...
- Parent → Child:
  ...
  - `REQUEST_REGISTER` (nearAccountId, registration options)
  - `REQUEST_SIGN` (nearAccountId, txSigningRequests, options)
- Child → Parent:
...
- Define TS types under `src/core/ServiceIframe/messages.ts`.

## WebAuthn Flow (No Popups)

- Parent calls `signTransactions()` inside a user gesture handler.
- Parent forwards `REQUEST_SIGN` to service iframe.
...
- Parent SDK
  - Add `ServiceIframeClient` (mount iframe, handshake, MessageChannel, request/response).
...
- Service Iframe (wallet)
  ...
  - Implement RPC server and handlers for `REQUEST_SIGN`/`REQUEST_REGISTER`.
