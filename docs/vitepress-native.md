...
This plan switches the site root to VitePress and embeds the existing React UI (Navbar, HomePage, Toaster) as Web Components on the VitePress homepage. It also removes React Router from the example app in favor of VitePress routing. A final section compares doing this with Vocs.
...
// Vite will transform this to a string at build time
// (requires vite to treat ?inline as raw import)
...
Notes:
- The wrapper uses `index.css?inline` to inject app styles inside the Shadow DOM so navbar/homepage look correct.
...
You can keep docs under `/docs/*` by adding a section index and nav entries, or keep all content at the root depending on preference.
...
## Testing & Pitfalls
- Shadow DOM CSS: we inject `index.css` into the custom element; if you add new app styles, they also need to be imported (or compose a dedicated `app-shell.css`).
...
## Would Vocs Be Easier?
Short answer: it depends on content authoring style.

- Why Vocs could be easier
  - Native MDX: you can import React components directly into pages without wrapping them as Web Components.
  - No Shadow DOM considerations: app styles/providers can be applied with regular React composition in MDX.
  - The guide at https://vocs.dev/docs/guides/components#custom-components shows straightforward React component usage.

- Why staying with VitePress is likely simpler here
  - VitePress is already integrated in this repo (see `examples/vite-secure/src/docs/.vitepress/*`) and proxied in Caddy.
  - We only need a small Web Component shell to reuse the existing React UI; no wholesale docs-site migration.
  - Current docs are already structured for VitePress, and you’ve begun adding theme enhancements and aliases.

- Recommendation
  - If your docs rely heavily on MDX interactivity across many pages, Vocs can reduce wrapper work. However, given the present setup, keeping VitePress and adding the `wallet-app` wrapper is the lowest-effort path to ship the homepage on VitePress quickly.
