...
function parseAndValidatePayload(payload: any): RegistrationCredentialConfirmationPayload {
  ...
  const intentDigest = assertString(payload.intentDigest, 'intentDigest');
...
export async function requestRegistrationCredentialConfirmation({
  ...
  return parseAndValidatePayload(res.payload as any);
}
