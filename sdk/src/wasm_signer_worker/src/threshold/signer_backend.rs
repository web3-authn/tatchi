...
impl Ed25519SignerBackend {
    pub fn from_encrypted_near_private_key(
        ...
        match signer_mode {
            SignerMode::LocalSigner => Ok(Self::Local(LocalEd25519Signer::from_encrypted_near_private_key(
                wrap_key,
                encrypted_private_key_data,
                encrypted_private_key_chacha20_nonce_b64u,
            )?)),
            SignerMode::ThresholdSigner => Ok(Self::Threshold(ThresholdEd25519RelayerSigner::unconfigured())),
        }
    }
...
impl ThresholdEd25519RelayerSigner {
    ...
    pub fn new(
        ...
        // If we don't have an externally provided mpcSessionId, we must have enough
        // context to call `/threshold-ed25519/authorize` on-demand.
        if normalized_mpc_session_id.is_none() {
            if vrf_challenge.is_none() {
                return Err("threshold-signer: missing vrfChallenge (required to authorize)".to_string());
            }
            if webauthn_authentication_json
                .as_ref()
                .map(|s| s.trim().is_empty())
                .unwrap_or(true)
            {
                return Err("threshold-signer: missing credential (required to authorize)".to_string());
            }
            if authorize_signing_payload_json
                .as_ref()
                .map(|s| s.trim().is_empty())
                .unwrap_or(true)
            {
                return Err("threshold-signer: missing signingPayload (required to authorize)".to_string());
            }
        }
    ...
    pub async fn sign(&self, message: &[u8]) -> Result<[u8; 64], String> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            // Prefer a provided mpcSessionId; otherwise authorize per signature.
            let mpc_session_id: String = if let Some(id) = cfg.mpc_session_id.as_deref() {
                id.to_string()
            } else {
                let vrf_challenge = vrf_challenge_opt.as_ref().ok_or_else(|| {
                    "threshold-signer: missing vrfChallenge (required to authorize)".to_string()
                })?;
                let credential_json = webauthn_authentication_json_opt.as_deref().ok_or_else(|| {
                    "threshold-signer: missing credential (required to authorize)".to_string()
                })?;
                let signing_payload_json = authorize_signing_payload_json_opt.as_deref();

                relayer_http::authorize_mpc_session_id(
                    cfg,
                    near_account_id,
                    purpose,
                    message,
            ...
            let client_sig_share = frost_ed25519::round2::sign(&signing_package, &client_nonces, client_key_package)
                .map_err(|e| format!("threshold-signer: round2 sign failed: {e}"))?;
            let client_sig_share_b64u = crate::encoders::base64_url_encode(&client_sig_share.serialize());

            let relayer_sig_share_b64u = match relayer_http::sign_finalize(
                cfg,
                &signing_session_id,
                &client_sig_share_b64u,
            )
            .await?
            {
            ...
            let relayer_sig_share = frost_ed25519::round2::SignatureShare::deserialize(&relayer_sig_share_bytes)
                .map_err(|e| format!("threshold-signer: invalid relayer signature share: {e}"))?;
            ...
            let relayer_verifying_share = frost_ed25519::keys::VerifyingShare::deserialize(&relayer_verifying_share_bytes)
                .map_err(|e| format!("threshold-signer: invalid relayer verifying share: {e}"))?;
...
fn parse_near_private_key_to_signing_key(private_key: &str) -> Result<ed25519_dalek::SigningKey, String> {
...
fn derive_client_key_package_from_wrap_key_seed(
    ...
    let signing_share_bytes = crate::threshold::threshold_client_share::derive_threshold_client_signing_share_bytes_v1(
        wrap_key,
        near_account_id,
    )?;
